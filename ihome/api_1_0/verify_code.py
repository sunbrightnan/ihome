...
def get_image_code(image_code_id):
    ...
    # redis：  字符串   列表  哈希   set
    # "key": xxx
    # 使用哈希维护有效期的时候只能整体设置 到了有效期所有的人验证码全没了
    ...
    # redis_store.set("image_code_%s" % image_code_id, text) 设置标号 值
    # redis_store.expire("image_code_%s" % image_code_id, constants.IMAGE_CODE_REDIS_EXPIRES) 设置有效期
    #                   记录名字                          有效期                              记录值
    try:
...
@api.route("/sms_codes/<re(r'1[02345678]\d{9}'):mobile>")
def get_sms_code(mobile):
    ...
    print(type(real_image_code),type(image_code))
